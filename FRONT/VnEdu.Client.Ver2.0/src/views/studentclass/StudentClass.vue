        ...
        <div class="bottom-studentclass" v-show="isShowStudentClass">
            ...
            <div class="between-schoolyear table-studet-class">
                <table class="m-table" border="1">
                    ...
                    <tbody>
                        <tr v-for="(student_class,i) in student_classs" :key="student_class.StudentId">
    ...
    methods: {
        ...
        clickBtnSee(){
            ...
            else if(m.classId == null || m.classId == "")
            {
                ...
                m.pageNumber = 1;
                axios
                .get(`${m.URLAPI}/api/v1/Student_Classs/GetPagingStudentClassByClassSemesterSchooYear?schoolYearId=${m.schoolyearId}&semesterId=${m.semesterId}&classId=${m.classId}&valueFilter=${m.SearchValue}&pageIndex=${m.pageNumber}&pageSize=${m.pageSize}`,
                    {'headers': { 'Authorization': 'Bearer ' + m.token }})
                .then(function (response){
                    if(response.data.IsError)
                    { 
        ...
        clickBtnExportExcel(){
            ...
            axios
            .get(`${m.URLAPI}/api/v1/Student_Classs/GetPagingStudentClassByClassSemesterSchooYear?schoolYearId=${m.schoolyearId}&semesterId=${m.semesterId}&classId=${m.classId}&pageIndex=${1}&pageSize=${1000}`,
                {'headers': { 'Authorization': 'Bearer ' + m.token }})
            .then(function (response){
                if(response.data.IsError)
                { 
                ...
                else
                {
                    ...
                    .map(item => [item.StudentCode, item.FullName, m.FomatGender(item.Gender), m.FomatDate(item.DateOfBirth), item.PhoneNumber, item.Email, item.Address]);

                    data.unshift(['Mã học sinh', 'Tên học sinh', 'Giới tính', 'Ngày sinh', 'Số điện thoại', 'Email', 'Địa chỉ']);
